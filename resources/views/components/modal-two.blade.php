<div
    class="block fixed top-0 left-0 w-max h-max overflow-y-auto overflow-x-hidden ease-in-out delay-150 modal_portfolio">
    <div class="transform-none max-w-xl my-7 mx-auto relative w-auto pointer-events-none">
        <div
            class="relative flex flex-col max-w-xl pointer-events-auto bg-cyan-200 bg-clip-padding border-2 border-grey-600 rounded-xl modal-fitting">
            <div class="flex shrink-0 items-center justify-between p-4 rounded-t-xl text-center"></div>
            <div class="text-center relative flex-auto p-4 pb-12">
                <div class="max-w-lg w-full px-3 mx-auto">
                    <div class="justify-center flex flex-wrap">
                        <div class="shrink-0 w-2/3 w-full px-3">
                            <!-- Portfolio Modal - Title-->
                            <h2 class="text-5xl leading-10">
                                Gazetteer</h2>
                            <!-- Icon Divider-->
                            <div class="front-page-divider-custom">
                                <div class="bg-black front-page-strip"></div>
                                <i class="fas fa-star text-5xl px-8"></i>
                                <div class="bg-black front-page-strip"></div>
                            </div>
                            <!-- Portfolio Modal - Image-->
                            <img class="border-solid border-2 border-black rounded-t-lg rounded-b-lg w-full mb-12"
                                 src="http://localhost:8080/public/images/project1.jpeg"
                                 alt="..."/>
                            <!-- Portfolio Modal - Text-->
                            <p class="mb-3">
                                Gazetteer project is a map of the world with a complete list of countries and
                                their associated land borders. The user experience begins with the project highlighting
                                the country you are accessing the webpage from. The dropdown menu will allow you to change
                                the country. While a country is highlighted you can use the provided buttons to select
                                information on the country as well as weather, Wikipedia search information and a newsfeed
                                on the capital. You can also click the pins to retrieve information on areas with a population
                                of over 20,000 and live web cameras
                            </p>
                            <div class="flex justify-center space-x-2">
                                <a target="_blank" href="{{asset("/under_construction")}}">
                                    <button
                                        class="text-slate-900 font-bold inline-block rounded px-6 pt-2.5 pb-2 uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-slate-50 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-slate-700 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-primary-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2) bg-green-600">
                                        Visit Gazetteer
                                    </button>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
